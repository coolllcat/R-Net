...
L2RAbdomen_MRCT_RGB_MAPPING_DICT = {
    'Liver': [230, 148, 34],
    'Spleen': [245, 245, 245],
    'Right_kidney': [119, 159, 176],
    # 'Left_kidney': [205, 62, 78],
} 
...
class L2RAbdomen_MRCT_Dataset(Dataset): 
    def __init__(self, args, is_train: bool, transforms=None) -> None:
        ...
        self.train_sample_dirs = [os.path.join(root_dir, sample_name) for sample_name in sample_names]  
        self.valid_test_sample_dirs = self.train_sample_dirs 
        print("L2RAbdomen MRCT data split: train: {}, valid and test: {}".format(len(self.train_sample_dirs), len(self.valid_test_sample_dirs))) 
    ...
    def __str__(self) -> str:
        return "L2RAbdomen_MRCT_Dataset"
...
if __name__ == "__main__": 
    ...
    parser.add_argument('--root_dir', type=str, default='/data/postgraduate/wmw/UniRegDatasets/Learn2Reg_Abdomen_MRCT', help='root dir where stores data') 
    ...
    train_transforms = RT.Compose([
        ...
        RT.RandomFlip(), 
        RT.Normalize(), 
        RT.AdjustChannels(L2RAbdomen_MRCT_LABEL_DICT),   
        RT.ToTensor() 
    ]) 
    ...
    train_dataset = L2RAbdomen_MRCT_Dataset(args, True, train_transforms) 
    valid_test_dataset = L2RAbdomen_MRCT_Dataset(args, False, valid_test_transforms) 
