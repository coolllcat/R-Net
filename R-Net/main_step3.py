...
def main(args): 
    ...
    # set model 
    model = Step_model_full(base_dim=args.base_dim, steps=args.steps, step=args.step)
    model.to(device) 
    ...
    for eopch in range(max_epoch): 
        for step, data in enumerate(train_loader): 
            ...
            moving_label = moving_label.to("cuda:1") 
            fixed_label = fixed_label.to("cuda:1")

            # image forward here 
            results = model(moving_image, fixed_image) 
            # ncc_loss = torch.Tensor([0.]).to("cuda:1")
            grad_loss = torch.Tensor([0.]).to("cuda:1")
            dice_loss = torch.Tensor([0.]).to("cuda:1")
            moving_image = moving_image.to("cuda:1") 
            fixed_image = fixed_image.to("cuda:1")
            for idx, result in enumerate(results):
                warped_label = utils.warp3d(moving_label, result) 
                grad_loss += losses.Gradient(result) 
                dice_loss += losses.Dice(warped_label, fixed_label)
            warped_image = utils.warp3d(moving_image, result[-1])
            ncc_loss = losses.Ncc(warped_image, fixed_image)

            loss = ncc_loss * 0 + grad_loss * args.lamda + dice_loss 
...
def test(args, model, test_loader):  
    for step, data in enumerate(test_loader): 
        ...
        # sitk.WriteImage(sitk.GetImageFromArray(flowabs.squeeze().cpu().numpy()), os.path.join(save_dir, 'disp_abs.nii.gz'))
        # if step >= 50:
        #     break
        # continue
        flow = results[-1]
...
if __name__ == "__main__": 
    ...
    parser.add_argument('--checkpoints_dir', type=str, default='./checkpoints_step3') 
