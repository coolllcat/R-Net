...
class Step_model_full(nn.Module):
    def __init__(self, base_dim = 64, steps = 15, field = 3, step = 1):
        super().__init__()
    ...
    def forward(self, x_1, x_2):
        ...
        self.flow_xyz = torch.tensor([0, -self.step, self.step]).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).to("cuda:0")
        flowresults = []
        for i in range(self.steps):
            flow_fea = flow_feas[:, i, :, :, :, :, :]
            flow_step = torch.sum(flow_fea * self.flow_xyz, dim=1)
            if i == 0:
                flowresults.append(flow_step)
                # flowabs = torch.sqrt(torch.sum(flow_step**2, dim=1, keepdim=True)) # For abs flow
            else:
                flowresults.append(warp3d(flowresults[-1], flow_step) + flow_step)
                # if i < 6:
                #     flowabs = warp3d(flowabs, flow_step) + torch.sqrt(torch.sum(flow_step**2, dim=1, keepdim=True)) # For abs flow
...
class Regress_model(nn.Module):
    def __init__(self, base_dim, steps, field=3):
        super().__init__()
        self.base_model = DeepReg(base_dim = base_dim)
        head_dim = steps * field * 3
        self.stephead = nn.Conv3d(base_dim, head_dim, 1, 1, 0)
        self.flowout = nn.Sequential(nn.Conv3d(head_dim, head_dim//2, 3, 1, 1),
                                     nn.LeakyReLU(0.2),
                                     nn.Conv3d(head_dim//2, head_dim//4, 3, 1, 1),
                                     nn.LeakyReLU(0.2),
...
if __name__ == '__main__':
    ...
    model = Step_model_distill(Step_model_full()).cuda()
    x = torch.randn((1,1,192,224,160)).cuda()
    tic = time()
