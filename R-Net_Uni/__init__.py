...
def get_transforms(which_set, is_resize, is_crop, target_shape): 
    """ Return an unified data transforms for all datasets. """ 
    channels_trans = None 
    channels_trans = RT.AdjustChannels(LABEL_DICT_MAPPING[which_set]) 

    resize_trans = None 
    if is_resize: 
        target_shape = tuple([int(x) for x in target_shape.split(',')]) 
...
def get_dataloader(args): 
    train_dataloaders = {}
    valid_test_dataloaders = {}

    for dataset in args.which_set:
        args.root_dir = os.path.join(args.dataroot, DIR_NAME_MAPPING[dataset]) 
        train_trans, valid_test_trans = get_transforms(dataset, args.is_resize, args.is_crop, args.target_shape) 
        train_dataset = DATASET_MAPPING[dataset](args, True, train_trans) 
        valid_test_dataset = DATASET_MAPPING[dataset](args, False, valid_test_trans) 

        train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, sampler=InfiniteRandomSampler(train_dataset)) 
        valid_test_dataloader = DataLoader(valid_test_dataset, batch_size=1, shuffle=False) 

        train_dataloaders.update({dataset: iter(train_dataloader)})
        valid_test_dataloaders.update({dataset: valid_test_dataloader})

    return train_dataloaders, valid_test_dataloaders
