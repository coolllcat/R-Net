...
from model import DeepReg, Step_model_full, Step_model_plus
import utils 
import losses
...
def main(args): 
    ...
    # set model 
    model1 = Step_model_full()
    model2 = Step_model_full()
    model3 = Step_model_full()
    model1.load_state_dict(torch.load('./checkpoints_dim64_step20_Brain/best_model.pth', map_location='cpu'))
    model2.load_state_dict(torch.load('./checkpoints_dim64_step20_Lung/best_model.pth', map_location='cpu'))
    model3.load_state_dict(torch.load('./checkpoints_dim64_step20_Abdomen/best_model.pth', map_location='cpu'))
    model = Step_model_plus(basemodels = [model1.base_model.cuda(), model2.base_model.cuda(), model3.base_model.cuda()])
    model.to(device) 
...
if __name__ == "__main__": 
    ...
    parser.add_argument('--checkpoints_dir', type=str, default='./checkpoints_dim64_step20_Uni') 
