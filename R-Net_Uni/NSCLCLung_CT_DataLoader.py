...
NSCLCLung_CT_LABEL_DICT = {
    'Lung': [1],
}


NSCLCLung_CT_RGB_MAPPING_DICT = {
    'Lung': [230, 148, 34],
} 
...
class NSCLCLung_CT_Dataset(Dataset): 
    def __init__(self, args, is_train: bool, transforms=None) -> None:
        ...
        print("NSCLCLung CT data split: train: {}, valid and test: {}".format(len(self.train_sample_dirs), len(self.valid_test_sample_dirs))) 
...
if __name__ == "__main__": 
    ...
    train_transforms = RT.Compose([
        ...
        RT.RandomFlip(), 
        RT.Normalize(), 
        RT.AdjustChannels(NSCLCLung_CT_LABEL_DICT),   
        RT.ToTensor() 
    ]) 
    ...
    train_dataset = NSCLCLung_CT_Dataset(args, True, train_transforms) 
    valid_test_dataset = NSCLCLung_CT_Dataset(args, False, valid_test_transforms) 
