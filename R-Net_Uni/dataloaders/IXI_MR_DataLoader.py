...
class IXI_MR_Dataset(Dataset): 
    def __init__(self, args, is_train: bool, transforms=None) -> None:
        ...
        if self.is_atlas: 
            ...
            print("IXI MR data split: train: {}, valid and test: {}".format(num_train, num_valid_test)) 
            ...
            print("IXI MR data split: train: {}, valid and test: {}".format(len(self.sample_pairs_train), len(self.sample_pairs_valid_test)))
...
if __name__ == "__main__": 
    ...
    parser.add_argument('--root_dir', type=str, default='/data/postgraduate/wmw/UniRegDatasets/IXI_Brain_MR', help='root dir where stores data') 
    ...
    train_transforms = RT.Compose([
        ...
        RT.RandomFlip(), 
        RT.Normalize(), 
        RT.AdjustChannels(IXI_MR_LABEL_DICT),  
        RT.ToTensor() 
    ]) 
    ...
    train_dataset = IXI_MR_Dataset(args, True, train_transforms) 
    valid_test_dataset = IXI_MR_Dataset(args, False, valid_test_transforms) 
